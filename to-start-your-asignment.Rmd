...
As the 20% of my mark for Macroeconomtrics depends on the VAR asignment and my participation in the e-meetings, I'm going to start doning my asignment. 

For my asignmente I'm going to estimate a VAR model with Eurostat data. I downloaded the data using the `to-download-my_data.Rmd` file   the `xx/04/2020`. Big thanks to the the `eurostat` package.

And remenber, as the lecturer said repeatedly during the e-meetings, that there are some rules to deliver the asignment to him:

- you have to deliver the asignment by mail to `pedro.j.perez@uv.es` **BEFORE the 15/05/2020**.   
 
- you should attach in you mail a **single .zip file**.   

- this single .zip file has to contain your **asignment as an Rproject**.   

- Remenber that you have to name the folder of you Rproject like: <VAR_country_1st-surname_name>. That is, if for instance, in your asignment you have used data from `Spain` and your name would be "Pedro Pérez Vázquez", then, the folder of your Rproject should be named: `VAR_perez_pedro_spain`. Please, all in lower case, except VAR, and **no spaces, no punctuation marks** in the name of the folder.   
...
data <- read_csv("./datos/my_data_ES.csv") 
```

Other way to load the data is: 


```{r}
data <- read_csv(here::here("datos", "my_data_ES.csv")) 
```

This second way to load teh data is preferable
...
```{r}
data <- data %>% select(time, Vol_us, Vol_15, Vol)
```

Droping rows with NAs to obtain the The common sample for my data.
